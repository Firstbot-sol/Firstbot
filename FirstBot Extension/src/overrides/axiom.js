(function () {
  ...
  window.MutationObserver = class extends previousMutationObserver {
    constructor(callback, options) {
      if (callback.toString().toLowerCase().includes("visibility") || callback.toString().toLowerCase().includes("decoy") || callback.toString().toLowerCase().includes("width") || callback.toString().toLowerCase().includes("96FF98")) {
        callback = () => { };
      }
