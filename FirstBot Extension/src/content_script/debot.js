...
chrome.runtime.onMessage.addListener(async function (request) {
  ...
  if (request.message !== "debot-token" && request.message !== "reset" && request.message !== "quickBuyAmount") {
    currentPage = "other";
    const tables = await findDebotTables();
    if (!tables || tables.length === 0) return;
    if (request.message === "debot-empty-tab") {
      request.message = tables.length > 1 ? "debot-meme" : "debot-home";
    }
    if (request.message === "debot-home") {
      currentPage = "debot-home";
      handleDebotHome();
    }
    if (request.message === "debot-meme") {
      currentPage = "debot-meme";
      if (tables && tables.length > 0) {
        tables.forEach((table) => {
          const allRows = Array.from(table.querySelectorAll('div[data-item-index]'));
          allRows.forEach((r) => addMemeButtons(r, tables.indexOf(table)));
        });
        tables.forEach((table) => {
          const observer = new MutationObserver((m) => {
            m.forEach((mutation) => {
              if (mutation.addedNodes.length && Array.from(mutation.addedNodes).concat(Array.from(mutation.removedNodes)).some((n) => n?.classList?.contains("bloom-buy-qt-btn") || n?.classList?.contains("bloom-snipe-qt-btn"))) {
                return
              }
              const addedRows = Array.from(mutation.addedNodes)
                .filter((r) => r.getAttribute && r.getAttribute("data-item-index"));
              if (!addedRows.length && !mutation.removedNodes.length) {
                const allRows = Array.from(table.querySelectorAll('div[data-item-index]'));
                allRows.forEach((r) => addMemeButtons(r, tables.indexOf(table)));
              } else {
                addedRows.forEach((r) => addMemeButtons(r, tables.indexOf(table)));
              }
            });
          });
  ...
  } else if (request.message === "debot-token") {
    currentPage = "debot-token";
    addBloomQuickPanel(request.url);
  } else if (request.message === "reset" && currentPage === "debot-token") {
    addBloomQuickPanel();
  } else if (request.message !== "quickBuyAmount") {
    currentPage = "other";
  } else if (request.message === "quickBuyAmount") {
    if (currentPage === "debot-meme") {
      const tables = await findDebotTables();
      if (!tables || tables.length === 0) return;
      tables.forEach((table) => {
        const allRows = Array.from(table.querySelectorAll('div[data-item-index]'));
        allRows.forEach((r) => addMemeButtons(r, tables.indexOf(table), true));
      });
    } else if (currentPage === "debot-home") {
...
async function handleDebotHome() {
  const tables = await findDebotTables();
  if (!tables || tables.length === 0) return;
  const container = tables[0];
  handleDebotHomeTable(container);

  const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      if (mutation.addedNodes.length && mutation?.target?.id === "scrollbar") {
        handleDebotHomeTable(mutation.addedNodes[0]);
      }
    });
  });
...
async function handleDebotHomeTable(container) {
...
async function findDebotTables(timeout = 5000) {
...
function addTrendingButtons(row, reload = false) {
  ...
  const anchor = row.querySelector("a[href*='/token/solana/']");
  if (!anchor) return;
  const tokenMint = anchor.href.split("/token/solana/")[1]?.split("?")?.[0];
  if (!tokenMint) return;

  const actionArea = row.querySelector("svg.tabler-icon-bolt");
  if (!actionArea) return;

  const actionParent = actionArea.parentElement;
  if (!actionParent) return;

  addTrendingBuyButton(tokenMint, actionParent);
}

function addMemeButtons(card, index, reload = false) {
  try {
    const migrationText = Array.from(card.querySelectorAll("div")).find(
      (div) => {
        return div.textContent === "100%"
      },
    );
    ...
    const actionArea = card.querySelector("svg.tabler-icon-bolt");
    if (!actionArea) return;

    const actionParent = actionArea.parentElement;
    if (!actionParent) return;

    if (isMigrating && index !== 2) {
      addMemeSnipingButton(actionParent, tokenMint);
    } else {
      addMemeBuyButton(actionParent, tokenMint);
    }
  } catch (error) {
    console.error("Failed to add Debot meme buttons:", error);
  }
}

function addMemeSnipingButton(actionParent, tokenMint) {
  const bloomButton = document.createElement("button");
  bloomButton.classList.add("bloom-snipe-qt-btn");
  for (const className of actionParent.classList) {
  ...
  insertBefore(actionParent, bloomButton);
}

function addMemeBuyButton(actionParent, tokenMint) {
  const bloomButton = document.createElement("button");
...
function addTrendingBuyButton(tokenMint, actionParent) {
  const bloomButton = document.createElement("button");
  bloomButton.classList.add("bloom-buy-qt-btn");
  for (const className of actionParent.classList) {
    bloomButton.classList.add(className);
...
function addTokenButtons(container, url) {
  try {
    ...
    let tokenMint = url.split("token/solana/")[1]?.split("?")?.[0];
    ...
    console.error("Failed to add Debot token buttons:", error);
  }
}
