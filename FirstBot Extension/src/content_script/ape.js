...
chrome.runtime.onMessage.addListener(async function (request) {
  ...
  if (request.message === "ape-gems") {
    currentPage = "ape-gems";
    const container = await findGemsContainer();
    if (container) {
      addGemsQTButtons();
      const observer = new MutationObserver(() => addGemsQTButtons());
      observer.observe(container, { childList: true, subtree: true });
    }
  } else if (request.message === "ape-token") {
    currentPage = "ape-token";
    ...
    if (buySellContainer) {
        ...
        for (const m of mutations) {
          if (
            Array.from(m.addedNodes)
              .concat(m.removedNodes)
              .find((nodes) =>
                Array.from(nodes).find((n) =>
                  n?.innerText?.includes("undergoing a liquidity migration"),
                ),
              )
          ) {
            addCustomBuyButton();
      ...
      observer.observe(buySellContainer, { childList: true });
    }
  } else if (request.message === "ape-home") {
    currentPage = "ape-home";
    const container = await findMain();
    if (container) {
      addNewTokensButtons();
      const observer = new MutationObserver(() => addNewTokensButtons());
      observer.observe(container, { childList: true });
    }
  } else if (request.message === "reset" && currentPage === "ape-token") {
    addBloomQuickPanel();
...
async function findMain(timeout = 5000) {
  for (let i = 0; i < timeout / 500; i++) {
    const main = document.querySelector("div.grid.w-full");
    if (main) return main;
...
async function findBuySellContainer(timeout = 5000) {
  for (let i = 0; i < timeout / 500; i++) {
    const container = document.querySelector("div.w-full.overflow-hidden");
    if (container) {
      return container;
    }
    await new Promise((r) => setTimeout(r, 500));
...
function addCustomBuyButton() {
  try {
    const isMigrating = Array.from(document.querySelectorAll("span.flex")).find(
      (span) => span.innerText === "Migrating...",
    );

    const existingBuyButton = document.querySelector(".bloom-buy-qt-btn");
    const existingSnipeButton = document.querySelector(".bloom-snipe-qt-btn");

    if (existingBuyButton && isMigrating) {
      existingBuyButton.remove();
    } else if (existingSnipeButton && !isMigrating) {
      existingSnipeButton.remove();
    } else if (existingBuyButton || existingSnipeButton) {
      return;
    }

    const actionArea = document.querySelector("button.w-full.font-semibold");
    ...
    if (isMigrating) {
      ...
      Array.from(actionArea.classList)
        .filter((c) => !c.includes("disabled") && !c.includes("opacity"))
        .forEach((c) => bloomButton.classList.add(c));
      bloomButton.style.border = "1px solid #96FF98";
      bloomButton.style.background = "none";
      bloomButton.style.setProperty("--un-ring-color", "#96FF98");
...
function addGemsQTButtons() {
  try {
    const cards = Array.from(document.querySelectorAll("div.h-20"));
    cards.forEach((card) => {
      const isMigrating = Array.from(card.querySelectorAll("div.text-xs")).some(
        (div) => div.textContent === "Migrating",
      );
      ...
      const poolLink = card.querySelector('a[preload="false"]');
      if (!poolLink) return;

      const actionArea = card.querySelector("button.text-xs");
      if (!actionArea) return;

      const buttonClass = isMigrating
        ? "bloom-snipe-qt-btn"
        : "bloom-buy-qt-btn";
      const buttonText = isMigrating ? "🚀 Snipe" : "🚀 Buy";
      ...
      bloomButton.onclick = function (event) {
        ...
        chrome.runtime.sendMessage({
          message: "openTab",
          url: `https://t.me/BloomSolana_bot?start=ref_QT_${isMigrating ? "sniper" : "ca"
            }_${tokenMint}`,
        });
      };
...
function addNewTokensButtons() {
  try {
    const cards = Array.from(document.querySelectorAll("div.group.contents"));
    cards.forEach((card) => {
      const existingBuyButton = card.querySelector(".bloom-buy-qt-btn");
...
async function updateBloomModalUI(preset, bodyWrapper) {
  ...
  manualBuyBtn.addEventListener("click", async () => {
    showButtonClickFeedback(manualBuyBtn, buyText, "buy", "pending");

    const val = manualBuyInput.value.trim();
    if (!val || isNaN(val) || val <= 0) {
      showToast("Please enter a valid SOL amount!");
      showButtonClickFeedback(manualBuyBtn, buyText, "buy", "error");
      return;
    }
    const token = await getBloomToken();
    if (!token) {
    ...
    if (buyOrder?.status === "success") {
      showToast("Buy order sent!");
      showButtonClickFeedback(manualBuyBtn, buyText, "buy", "success");
    } else if (buyOrder?.status === "timeout") {
      showToast("Error sending order, try switching region!");
      showButtonClickFeedback(manualBuyBtn, buyText, "buy", "error");
    } else {
  ...
  manualSellBtnPercent.addEventListener("click", async () => {
    showButtonClickFeedback(manualSellBtnPercent, sellText, "sell", "pending");

    const val = manualSellInputPercent.value.trim();
    if (!val || isNaN(val) || val <= 0 || val > 100) {
      showToast("Please enter a valid percentage (1 - 100)!");
      showButtonClickFeedback(manualSellBtnPercent, sellText, "sell", "error");
      return;
    }
    const token = await getBloomToken();
    if (!token) {
    ...
    if (sellOrder?.status === "success") {
      showToast("Sell order sent!");
      showButtonClickFeedback(manualSellBtnPercent, sellText, "sell", "success");
    } else if (sellOrder?.status === "timeout") {
      showToast("Error sending order, try switching region!");
      showButtonClickFeedback(manualSellBtnPercent, sellText, "sell", "error");
    } else {
  ...
  manualSellBtnSol.addEventListener("click", async () => {
    showButtonClickFeedback(manualSellBtnSol, sellText, "sell", "pending");

    const val = manualSellInputSol.value.trim();
    if (!val || isNaN(val) || val <= 0) {
      showToast("Please enter a valid SOL amount!");
      showButtonClickFeedback(manualSellBtnSol, sellText, "sell", "error");
      return;
    }
    const token = await getBloomToken();
    if (!token) {
    ...
    if (sellOrder?.status === "success") {
      showToast("Sell order sent!");
      showButtonClickFeedback(manualSellBtnSol, sellText, "sell", "success");
    } else if (sellOrder?.status === "timeout") {
      showToast("Error sending order, try switching region!");
      showButtonClickFeedback(manualSellBtnSol, sellText, "sell", "error");
    } else {
