...
chrome.runtime.onMessage.addListener(async function (request) {
  ...
  if (request.message === "discord") {
    quickBuyAmount =
      (await chrome.storage.local.get("bloom.quickBuyAmount"))?.["bloom.quickBuyAmount"] || 0.5;
    const scraperSettings =
      (await chrome.storage.local.get("bloom.scraperSettings"))?.["bloom.scraperSettings"] || {};
    autoBuyAmount = scraperSettings.discordAutoBuyAmount || quickBuyAmount;
    currentPage = "discord";
    currentObservers.forEach((observer) => observer.disconnect());
    currentObservers = [];
    const titleContainer = await findTitleContainer();
    if (!titleContainer) return;
    addAutoBuyToggleButton(titleContainer);
    const allMessages = Array.from(document.body.querySelectorAll('li[class*="messageListItem"]'));
    allMessages.forEach((message) => {
      const splittedMessageId = message.getAttribute('id')?.split('-');
      const messageId = splittedMessageId[splittedMessageId.length - 1];
      if (!messageId) return;
    ...
    const observer = new MutationObserver((mutations) => {
      for (const m of mutations) {
        m?.addedNodes?.forEach((node) => {
          if (node?.nodeName?.toLowerCase() === 'li' && Array.from(node.classList).some(c => c.includes('messageListItem'))) {
            const splittedMessageId = node.getAttribute('id')?.split('-');
            const messageId = splittedMessageId[splittedMessageId.length - 1];
            if (!messageId) return;
            processMessage(node, messageId);
            const observer = new MutationObserver((mutations) => {
              for (const m of mutations) {
                m?.addedNodes?.forEach((node) => {
                  if (node?.nodeName?.toLowerCase() === 'article' && Array.from(node.classList).some(c => c.includes('embedWrapper'))) {
                    const element = node.closest('li[class*="messageListItem"]');
                    if (!element) return;
                    const splittedMessageId = element.getAttribute('id')?.split('-');
                    const messageId = splittedMessageId[splittedMessageId.length - 1];
                    if (!messageId) return;
                    processMessage(element, messageId);
                  }
                });
              }
            });
  ...
  if (request.message === "quickBuyAmount") {
    ...
    if (currentPage === "discord") {
      const titleContainer = await findTitleContainer();
      if (!titleContainer) return;
      const allMessages = document.body.querySelectorAll('li[class*="messageListItem"]');
      allMessages.forEach((message) => {
        const splittedMessageId = message.getAttribute('id')?.split('-');
        const messageId = splittedMessageId[splittedMessageId.length - 1];
        if (!messageId) return;
...
async function findTitleContainer(timeout = 5000) {
  for (let i = 0; i < timeout / 500; i++) {
    const container = document.querySelector('section[class*="title"]');
    if (container && container?.firstElementChild?.firstElementChild) return container.firstElementChild.firstElementChild;
    await new Promise((r) => setTimeout(r, 500));
  }
  return null;
}

function processMessage(element, messageId) {
  try {
    const existingButtonContainers = element.querySelectorAll(".bloom-button-container");
    existingButtonContainers.forEach((c) => {
      c.remove();
    });

    for (const child of Array.from(element.childNodes)) {
      transformNode(child, messageId);
    }
  } catch (error) {
  ...
  async function handleAutoBuy(address) {
    if (autoBuyStartedAt && !seenMessages.includes(messageId)) {
      ...
      const messageDate = convertSnowflakeToDate(messageId);
      if (messageDate <= new Date(autoBuyStartedAt)) {
        return;
      }

      const timeElapsed = Date.now() - messageDate.getTime();
  ...
  function transformNode(node) {
    if (node.nodeType === Node.ELEMENT_NODE && node.closest('div[class*="repliedTextPreview"]')) {
      return;
    }
    ...
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      if (nodeName === "a") {
        ...
        const textMatch = anchorText.match(ADDRESS_REGEX);
        if (textMatch?.length) {
          textMatch.forEach(async (address) => {
            handleAutoBuy(address);
      ...
      else if (nodeName === "article" && Array.from(node.classList).some(c => c.includes("embedWrapper"))) {
...
<image id="image0_8_20121" width="72" height="72" xlink:href="data:image/png;base64,"/>
    ...
    bloomButton.addEventListener("mouseenter", () => {
      bloomButton.style.background = "rgba(238, 167, 237, 0.1)";
    });

    bloomButton.addEventListener("mouseleave", () => {
      bloomButton.style.background = "transparent";
    });
    ...
    panelButton.style.cssText = bloomButton.style.cssText;
  ...
  `;
    ...
    panelButton.style.height = "100%";
    panelButton.style.width = "100%";

    panelButton.addEventListener("mouseenter", () => {
      panelButton.style.background = "rgba(238, 167, 237, 0.1)";
    });
...
async function addBloomQuickPanel(anchorButton, tokenMint) {
  try {
    ...
    console.error("Failed to inject Bloom popup:", error);
  }
}
