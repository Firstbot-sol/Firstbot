...
chrome.runtime.onMessage.addListener(async function (request) {
  ...
  if (request.message !== "gmgn-token" && request.message !== "reset" && request.message !== "quickBuyAmount") {
    currentPage = "other";
    const tables = await findGmgnTables();
    if (!tables || tables.length === 0) return;
    if (request.message === "gmgn-empty-tab") {
      request.message = tables.length > 1 ? "gmgn-meme" : "gmgn-home";
    }

    if (request.message === "gmgn-home") {
      currentPage = "gmgn-home";
      const container = tables[0];
      if (container) {
        const rows = Array.from(container.querySelectorAll(".g-table-row"));
        rows.forEach((r) => addTrendingButtons(r));
        const observer = new MutationObserver((mutations) => {
          mutations.forEach((mutation) => {
            mutation.addedNodes.forEach((node) => {
              addTrendingButtons(node)
            });
          });
        });
    ...
    if (request.message === "gmgn-meme") {
      currentPage = "gmgn-meme";
      if (tables && tables.length > 0) {
        const allRows = Array.from(document.querySelectorAll(".g-table-row"));
        allRows.forEach((r) => addMemeButtons(r));
        tables.forEach((table) => {
          const observer = new MutationObserver((m) => {
            const addedRows = Array.from(m)
              ...
              .flat()
              .filter((r) => r.classList?.contains("g-table-row"));
            if (addedRows.length === 0) {
              const allCurrentRows = Array.from(
                document.querySelectorAll(".g-table-row"),
              );
              allCurrentRows.forEach((r) => addMemeButtons(r));
            } else {
              addedRows.forEach((r) => addMemeButtons(r));
            }
          });
  ...
  } else if (request.message === "gmgn-token") {
    currentPage = "gmgn-token";
    const container = await findTokenContainer();
    if (container) {
      addTokenButtons(container, request.url);
      addBloomQuickPanel(request.url);
    }
  } else if (request.message === "reset" && currentPage === "gmgn-token") {
    addBloomQuickPanel();
  } else if (request.message !== "quickBuyAmount") {
    currentPage = "other";
  } else if (request.message === "quickBuyAmount") {
    if (currentPage === "gmgn-meme") {
      const allRows = Array.from(document.querySelectorAll(".g-table-row"));
      allRows.forEach((r) => addMemeButtons(r, true));
    } else if (currentPage === "gmgn-home") {
      const rows = Array.from(document.querySelectorAll(".g-table-row"));
      rows.forEach((r) => addTrendingButtons(r, true));
    }
  }
});

async function findGmgnTables(timeout = 5000) {
  for (let i = 0; i < timeout / 500; i++) {
    const tables = Array.from(
      document.querySelectorAll(".g-table-tbody-virtual-holder-inner"),
    );
    if (tables.length > 0) {
      return tables;
    }
...
async function findTokenContainer(timeout = 5000) {
  for (let i = 0; i < timeout / 500; i++) {
    const center = document.querySelector("#tokenCenter");
    if (center) return center.nextElementSibling;
    await new Promise((r) => setTimeout(r, 500));
  }
  return null;
}

function addTrendingButtons(row, reload = false) {
  const existingBuyButton = row.querySelector(".bloom-buy-qt-btn");

  if (existingBuyButton && !reload) {
    return;
  } else if (existingBuyButton) {
    existingBuyButton.remove();
  }

  const rowDataKey = row.getAttribute("data-row-key");
  const tokenMint = rowDataKey.includes("_")
    ? rowDataKey.split("_")[1]
    : rowDataKey;
  if (!tokenMint) return;

  const actionArea = row.querySelector(".g-table-cell-fix-right-first");
  if (!actionArea) return;
  const lastChild = actionArea.lastChild;
  if (!lastChild) return;
  const classDiv = Array.from(
    row.querySelectorAll('div[datatype="out"]'),
  ).find((div) => !div.querySelector("a[href]"));
  if (!classDiv) return;

  const currentStyle = actionArea.getAttribute("style");
  actionArea.setAttribute(
    "style",
    currentStyle
      ? `${currentStyle}; flex-direction: column; width: 100%;`
      : "flex-direction: column; width: 100%;",
  );

  addTrendingBuyButton(lastChild, tokenMint, classDiv);
}

function addMemeButtons(card, reload = false) {
  try {
    const migrationText = Array.from(card.querySelectorAll("div")).find(
      (div) => {
        return Array.from(div.childNodes)
          .filter((node) => node.nodeType === Node.TEXT_NODE)
          .some((node) => node.textContent.trim() === "Seeding");
      },
    );
    const isMigrating = !!migrationText;
    ...
    const rowDataKey = card.getAttribute("data-row-key");
    const tokenMint = rowDataKey.includes("_")
      ? rowDataKey.split("_")[1]
      : rowDataKey;
    if (!tokenMint) return;

    let actionArea = card.querySelector('div[datatype="out"]');
    let classDiv = actionArea;
    if (!actionArea) {
      actionArea = migrationText;
      classDiv = Array.from(
        document.querySelectorAll('div[datatype="out"]'),
      ).find((d) =>
        Array.from(d.childNodes).some(
          (c) => c.nodeName.toLowerCase() === "svg",
        ),
      );
    }
    if (!actionArea) return;
    ...
    if (isMigrating) {
      addMemeSnipingButton(actionArea, classDiv, tokenMint);
    } else {
      addMemeBuyButton(actionArea, classDiv, tokenMint);
    }
  } catch (error) {
    console.error("Failed to add GMGN meme buttons:", error);
  }
}

function addMemeSnipingButton(actionArea, classDiv, tokenMint) {
  const bloomButton = document.createElement("button");
  bloomButton.classList.add("bloom-snipe-qt-btn", classDiv?.classList?.[0]);
  bloomButton.textContent = "🚀 Snipe";
  bloomButton.setAttribute("style", "margin-right: 6px;");
...
function addMemeBuyButton(actionArea, classDiv, tokenMint) {
  const bloomButton = document.createElement("button");
  bloomButton.classList.add("bloom-buy-qt-btn", classDiv?.classList?.[0]);
  bloomButton.textContent = `🚀 ${quickBuyAmount}`;
...
function addTrendingBuyButton(actionArea, tokenMint, classDiv) {
  const bloomButton = document.createElement("button");
  bloomButton.classList.add("bloom-buy-qt-btn", classDiv?.classList?.[0]);
  bloomButton.textContent = `🚀 ${quickBuyAmount}`;
  bloomButton.onclick = async function (event) {
...
function addTokenButtons(container, url) {
  try {
    const existingButton = container.querySelector(".bloom-buy-qt-btn");
    ...
    bloomButton.textContent = "🚀 Bloom";

    let tokenMint = url.split("token/")[1]?.split("?")?.[0];
    if (tokenMint?.includes('_')) {
      tokenMint = tokenMint?.split('_')?.[1];
    }
    if (!tokenMint) return;
    ...
    insertAfter(firstChild, bloomButton);
  } catch (error) {
    console.error("Failed to add GMGN token buttons:", error);
  }
}
...
async function addBloomQuickPanel(url) {
  try {
    const {
    ...
    if (!Array.isArray(bloomPresets) || bloomPresets.length === 0) {
      ...
      presetSelector.addEventListener("change", async (e) => {
        ...
        updateBloomModalUI(url, newPreset, bodyWrapper);
      });
    ...
    await updateBloomModalUI(url, chosenPreset, bodyWrapper);
...
function addHideUnhideBehavior(container, initialHiddenState, containerHeight) {
  ...
  document.addEventListener("keydown", (evt) => {
    if ((evt.ctrlKey || evt.metaKey) && evt.key.toLowerCase() === "b") {
      evt.stopPropagation();
      evt.preventDefault();
      console.log(isQuickPanelHidden)
      if (isQuickPanelHidden) {
        unhideContainer();
      } else {
        hideContainer();
...
async function updateBloomModalUI(url, preset, bodyWrapper) {
