...
let currentPage, quickBuyAmount;
chrome.runtime.onMessage.addListener(async function (request) {
  ...
  if (request.message === "pumpfun-board") {
    quickBuyAmount =
      (await chrome.storage.local.get("bloom.quickBuyAmount"))?.[
      "bloom.quickBuyAmount"
      ] || 0.5;
    currentPage = "pumpfun-board"
    const container = await findBoardContainer();
    if (container) {
      const allCards = Array.from(container.children);
      allCards.forEach((card) => {
        addBoardButtons(card);
      });
  ...
  } else if (request.message === "pumpfun-profile") {
    quickBuyAmount =
      (await chrome.storage.local.get("bloom.quickBuyAmount"))?.[
      "bloom.quickBuyAmount"
      ] || 0.5;
    currentPage = "pumpfun-profile"
    const container = await findProfileContainer();
    if (container) {
      addProfileButtons(container);
      const observer = new MutationObserver((mutations) => {
        if (Array.from(mutations).find((m) => m.addedNodes.length > 0 && Array.from(m.addedNodes).every((n) => n.nodeName.toLowerCase() !== 'button'))) {
          addProfileButtons(container)
        }
      });
      observer.observe(container, { childList: true, subtree: true });
    }
  } else if (request.message === "pumpfun-advanced") {
    quickBuyAmount =
      (await chrome.storage.local.get("bloom.quickBuyAmount"))?.[
      "bloom.quickBuyAmount"
      ] || 0.5;
    currentPage = "pumpfun-advanced"
    const main = await findMain();
    if (main) {
      const tBody = await findTableBody(main);
      if (tBody) {
        const tds = tBody.querySelectorAll('td.sticky');
        tds.forEach((td) => {
          const card = td.parentElement
          addAdvancedButton(card, td)
        })
      }
      const observer = new MutationObserver((mutations) => {
        for (const m of mutations) {
          if (m.addedNodes.length > 0) {
            const addedTds = Array.from(m.addedNodes).filter((n) => n.nodeName.toLowerCase() === 'td' && n.classList.contains('sticky'));
            addedTds.forEach((td) => {
              const card = td.parentElement
              addAdvancedButton(card, td)
            })
            const addedTrs = Array.from(m.addedNodes).filter((n) => n.nodeName.toLowerCase() === 'tr')
            addedTrs.forEach((tr) => {
              const td = tr.querySelector('td.sticky')
              addAdvancedButton(tr, td)
            })
          }
        }
      });
  ...
  } else if (request.message === "pumpfun-token") {
    currentPage = "pumpfun-token"
    addBloomQuickPanel();
    ...
    if (currentPage === "pumpfun-board") {
      const container = await findBoardContainer();
      if (container) {
        const allCards = Array.from(container.children);
        allCards.forEach((card) => {
          addBoardButtons(card, quickBuyAmount);
        });
      }
    } else if (currentPage === "pumpfun-profile") {
      const container = await findProfileContainer();
      if (container) {
        addProfileButtons(container);
      }
    } else if (currentPage === "pumpfun-advanced") {
      const main = await findMain();
      if (main) {
        const tBody = await findTableBody(main);
        if (tBody) {
          ...
          tds.forEach((td) => {
            const card = td.parentElement
            addAdvancedButton(card, td, quickBuyAmount);
          })
        }
      }
    }
  } else if (request.message === "reset" && currentPage === "pumpfun-token") {
    addBloomQuickPanel();
  } else {
    currentPage = "other";
  }
...
async function findBoardContainer(timeout = 5000) {
...
async function findProfileContainer(timeout = 5000) {
  for (let i = 0; i < timeout / 500; i++) {
    const container = document.querySelector('div.grid.items-start');
    if (container) return container;
    await new Promise((r) => setTimeout(r, 500));
  }
  return null;
...
async function findMain(timeout = 5000) {
  for (let i = 0; i < timeout / 500; i++) {
...
async function findTableBody(main, timeout = 5000) {
  for (let i = 0; i < timeout / 500; i++) {
    const container = main.querySelector('tbody[data-testid="virtuoso-item-list"]');
    if (container) return container;
    await new Promise((r) => setTimeout(r, 500));
  }
  return null;
...
function addBoardButtons(card) {
  try {
    const existingBuyButton = card.querySelector(".bloom-buy-qt-btn");
    if (existingBuyButton) existingBuyButton.remove();

    const poolLink = card.querySelector('a[href*="/coin/"]');
    if (!poolLink) return;

    const hfit = card.querySelector(".grid.h-fit");
    if (!hfit) return;
    const actionArea = hfit.firstElementChild;
    if (!actionArea) return;

    hfit.style.width = "100%";
    ...
    bloomButton.style.marginLeft = "auto";
    bloomButton.style.marginRight = "48px";
    bloomButton.style.borderRadius = "4px";
    bloomButton.style.padding = "4px 8px";
    const tokenMint = poolLink.href.split("/").pop();
    ...
    bloomButton.onclick = async function (event) {
      ...
      const order = await interactWithBloom(null, tokenMint, "swap", token, quickBuyAmount, "buy", null);
    ...
    actionArea.appendChild(bloomButton);
  } catch (error) {
    console.error("Failed to add Bloom button:", error);
  }
}

function addProfileButtons(container) {
  if (profileObservers.length) {
    profileObservers.forEach((observer) => observer.disconnect());
    profileObservers = [];
  }
  const lastChild = container.lastChild;
  const isHeldTab = lastChild.classList.contains("grid")
  const isCreatedTab = lastChild.classList.contains("max-w-[400px]")
  if (!isHeldTab && !isCreatedTab) return;

  if (isHeldTab) {
    function addCoinButton(coin) {
      coin.style = "display: flex; justify-content: space-between; align-items: center;";
      const existingBuyButton = coin.querySelector(".bloom-buy-qt-btn");
      if (existingBuyButton) existingBuyButton.remove();
      const tokenMint = coin.querySelector("a[href*='/coin/']")?.href.split("/").pop();
      if (!tokenMint) return;
      const bloomButton = document.createElement("button");
      ...
      coin.appendChild(bloomButton);
    }
    const allCoins = container.querySelectorAll("div.grid.w-fit");
    ...
    const observer = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.addedNodes.length > 0) {
          m.addedNodes.forEach((n) => addCoinButton(n));
        }
      }
    });
    observer.observe(lastChild, { childList: true, subtree: true });
    ...
    function addCoinButton(coin) {
      ...
      const tokenMint = coin.querySelector("a[href*='/coin/']")?.href.split("/").pop();
      if (!tokenMint) return;
      const hfit = coin.querySelector(".grid.h-fit");
      if (!hfit) return;
      const actionArea = hfit.firstElementChild;
      if (!actionArea) return;

      hfit.style.width = "100%";
      lastChild.classList.remove("max-w-[400px]");
      lastChild.classList.add("max-w-[440px]");
    ...
    const allCoins = container.querySelectorAll("div.group.relative");
    allCoins.forEach((coin) => {
      addCoinButton(coin);
    });
    const observer = new MutationObserver((mutations) => {
...
async function addAdvancedButton(card, td) {
  const existingBuyButton = card.querySelector(".bloom-buy-qt-btn");
  if (existingBuyButton) existingBuyButton.remove();

  const tokenMint = card.querySelector("a[href*='/coin/']")?.href.split("/").pop();
  if (!tokenMint) return;

  const currentBuyButton = td.querySelector('button')
  if (!currentBuyButton) return;

  const bloomButton = currentBuyButton.cloneNode(true);
  const p = document.createElement('p')
  p.textContent = `🚀`;
  p.classList.add('mr-2')
  const img = bloomButton.querySelector('img')
  if (!img) return;
  insertBefore(img, p)
  img.remove()
  const previousSpan = bloomButton.querySelector('span')
  if (!previousSpan) return;
  const span = previousSpan.cloneNode(true)
  span.textContent = `${quickBuyAmount}`;
  previousSpan.remove()
  bloomButton.classList.add("bloom-buy-qt-btn");
  bloomButton.style.background = 'none';
  bloomButton.style.border = "1px solid #96FF98";
  bloomButton.style.zIndex = 1000;
  bloomButton.style.borderRadius = "4px";
  bloomButton.style.marginTop = "12px";
  bloomButton.style.color = "white";
  bloomButton.style.padding = 0;
  ...
  bloomButton.appendChild(textWrapper)
  ...
  insertAfter(currentBuyButton, bloomButton);
}
...
async function addBloomQuickPanel() {
  try {
    ...
    if (!Array.isArray(bloomPresets) || bloomPresets.length === 0) {
      ...
      headerLeftText.textContent = "No First Presets";
    ...
    const divToInject = document.querySelector('div[role="dialog"]');
    if (divToInject) {
      divToInject.style.width = "100%"
      divToInject.style.maxWidth = "100%"
      insertBefore(divToInject.firstChild, container);
    } else {
    ...
    addHideUnhideBehavior(divToInject || document.body, container, hiddenState, containerHeight);
...
function addHideUnhideBehavior(target, container, initialHiddenState, containerHeight) {
  const originalTransition = "bottom 0.3s ease, right 0.3s ease";
  const showMargin = 48;
  ...
  if (!isQuickPanelHidden) {
    initializePanelDrag(target, container, `bloom.positionBeforeHide.${platform}`);
...
async function updateBloomModalUI(preset, bodyWrapper) {
  ...
  buyAmounts.forEach((amt) => {
    ...
    btn.addEventListener("click", async () => {
      ...
      const tokenMint = window.location.pathname.split("/").pop();
      if (!tokenMint) {
      ...
      const buyOrder = await interactWithBloom(chosenRegion, tokenMint, "swap", token, amt, "buy", preset);
...
function initializePanelDrag(target, panelRoot, storageKey) {
  ...
  function createDragOverlay(target) {
    ...
    target.appendChild(dragOverlay);
  }
